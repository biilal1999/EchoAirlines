...
public class SalidaReservaVueloIntent implements IntentRequestHandler {
    ...
    public boolean canHandle(HandlerInput handlerInput , IntentRequest intentRequest) {
        ...
        return (handlerInput.matches(intentName(Intents.RESERVA_VUELO_INTENT))
                && salida.getValue() == null && accion.getValue() == null);
    }
    ...
    public Optional<Response> handle(HandlerInput handlerInput, IntentRequest intentRequest) {
        ...
        String consulta = "SELECT * FROM vuelos WHERE origen=" + "'" + origen + "'" +
                ...
                precioMaximo + "'" + " and fecha=" + "'" + fecha + "'" + " and billetes>0";
        ...
        if (total == 0){
            bd.cerrarConexion();
            speechText = "No hay vuelos disponibles con estas características. Pruebe, si lo desea, " +
                    "a elegir otro día para reservar.";
            ...
            /*String consultaSegunda = "SELECT * FROM vuelos WHERE origen=" + "'" + origen + "'" +
                    " and destino=" + "'" + destino + "'" + " and precio<=" + "'" +
                    precioMaximo + "'" + " and fecha=" + "'" + fecha + "'";
            rsAux = bd.ejecutarConsulta(consultaSegunda);*/
            rsAux = bd.ejecutarConsulta(consulta);

            try{
                speechText = "Tenemos los siguientes vuelos disponibles para este día.";
                ...
                while (rsAux.next()){
                    //if (mes.equals(sdfMonth.format(rsAux.getDate("fecha")))){
                        if (rsAux.getDate("fecha").compareTo(hoy) > 0){
                            speechText += " Un vuelo a las " + sdfTime.format(rsAux.getTime("salida"))
                                    + " por " + rsAux.getFloat("precio") + " euros.";
                        }
                    //}
                }

                speechText += " Dígame la hora concreta para el que quiere reservar.";
